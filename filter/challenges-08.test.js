...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  let regex = /[aeiou]+/gi;
  let vowels = arr.filter(n => (n.match(regex)));
  return vowels
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let newArr = arr.filter(n => !forbiddenValues.includes(n));
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Solution code here...
  let newArr = arr.filter(n => (minBaseStat < n.baseStat));
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  let newArr = arr.filter(n => (minBaseStat < n.baseStat));
  let newArr2 = newArr.map(x => x.stat.name);
